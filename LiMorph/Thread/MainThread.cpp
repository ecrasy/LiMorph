...
namespace LiMorph {
    ...
    HWND MainThread::findMainWindow(unsigned long process_id)
    {
        handle_data data;
        data.process_id = process_id;
        data.window_handle = 0;
        EnumWindows(MainThread::enumWindowsCallback, (LPARAM)&data);
        return data.window_handle;
    }


    BOOL CALLBACK MainThread::enumWindowsCallback(HWND handle, LPARAM lParam) {
    ...
    BOOL MainThread::isMainWindow(HWND handle)
    {
